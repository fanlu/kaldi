  ...
  try {
    ...
    if (po.NumArgs() == 2) {
      ...
      for (; !spk2utt_reader.Done(); spk2utt_reader.Next()) {
        ...
        int32 utt_count = 0;
        std::vector<std::string> uttlist_real;
        for (size_t i = 0; i < uttlist.size(); i++) {
          ...
          if (!ivector_reader.HasKey(utt)) {
            ...
            uttlist_real.push_back(utt);
          }
        }
        if (uttlist_real.size() == 0) {
          KALDI_WARN << "Not producing output for speaker " << spk
                     << " since no utterances had iVectors";
          num_spk_err++;
        } else {
          std::vector<int64> rand_index;
          for (int64 i = 0; i < uttlist_real.size(); ++i)
            rand_index.push_back(i);
          if (uttlist_real.size() < concat_num) {
            for (int64 i = 0; i < concat_num - uttlist_real.size(); i++) {
              rand_index.push_back(i % uttlist_real.size());
            }
          }
          std::random_shuffle(rand_index.begin(), rand_index.end());
          // for (int64 i = 0; i < concat_num; i++) {
          //   int64 try_i = rand() % uttlist_real.size();
          //   rand_index.push_back(try_i);
          // }
          Vector<BaseFloat> spk_concat;
          int dim = 0;
          for (int64 i = 0; i < concat_num; i++) {
            int64 real_i = rand_index[i];
            std::string utt = uttlist[real_i];
            if (utt_count == 0) {
              spk_concat = ivector_reader.Value(utt);
              dim = spk_concat.Dim();
              // std::cout << (spk_concat)(0) << " " << std::endl;
              spk_concat.Resize(dim * concat_num, kCopyData);
              // std::cout << (spk_concat)(0) << " " << std::endl;
            } else {
              memcpy(spk_concat.Data() + utt_count * dim,
                     ivector_reader.Value(utt).Data(), sizeof(BaseFloat) * dim);
            }
            // std::cout << (ivector_reader.Value(utt))(0) << " " << std::endl;
            // std::cout << spk_concat(0) << std::endl;
            // std::cout << " " << spk_concat(600) << std::endl;
            // std::cout << " " << spk_concat(1200) << std::endl;
            num_utt_done++;
            utt_count++;
          }
          // spk_mean.Scale(1.0 / utt_count);
          ivector_writer.Write(spk, spk_concat);
          if (num_utts_wspecifier != "") num_utts_writer.Write(spk, utt_count);
          num_spk_done++;
          spk_sumsq += VecVec(spk_concat, spk_concat);
          if (spk_sum.Dim() == 0) spk_sum.Resize(spk_concat.Dim());
          spk_sum.AddVec(1.0, spk_concat);
        }
      }
