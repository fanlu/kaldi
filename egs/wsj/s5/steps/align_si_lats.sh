...
generate_ali_from_lats=true # If true, alingments generated from lattices.
...
if [ $stage -le 3 ]; then
  ...
  echo "$cmd JOB=1:$nj $dir/log/generate_lattices.JOB.log gmm-latgen-faster --acoustic-scale=$acoustic_scale --beam=$final_beam --lattice-beam=$final_beam --allow-partial=false --word-determinize=false \"$mdl_cmd\" \"ark:gunzip -c $dir/fsts.JOB.gz|\" \"$feats\" \"ark:|gzip -c >$dir/lat.JOB.gz\""
